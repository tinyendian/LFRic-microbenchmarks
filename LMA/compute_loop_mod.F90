...
contains
  ...
  subroutine compute_loop_tiled(ncolours, tile_x, tile_y, ncell_3d, nlayers, ndf1, undf1, ndf2, undf2, &
    ...
    integer(kind=i_def) :: count, colour, ntiles_per_colour, tile, cell
    integer(kind=i_def) :: start_cell, stop_cell
    ...
    do count = 1, 1000
       do colour = 1, ncolours
          ...
          if (mod(ncells_per_colour(colour), tile_x*tile_y) .ne. 0) then
             write(*,'(A)') 'Number of cells per colour must evenly divide by tile size'
             stop
          end if
          ntiles_per_colour = ncells_per_colour(colour)/(tile_x*tile_y)

          !$omp parallel default(shared), private(tile, start_cell, stop_cell, cell)
          !$omp do schedule(static)
          do tile = 1, ntiles_per_colour
             start_cell = (tile-1)*tile_x*tile_y + 1
             stop_cell = tile*tile_x*tile_y

             do cell=start_cell, stop_cell
  ...
  subroutine compute_loop_inlined(ncolours, tile_x, tile_y, ncell_3d, nlayers, ndf1, undf1, ndf2, undf2, &
    ...
    integer(kind=i_def) :: count, colour, ntiles_per_colour, tile, cell
    integer(kind=i_def) :: start_cell, stop_cell, k, ik, df, df2, m1, m2
    ...
    do count = 1, 1000
       do colour = 1, ncolours
          ...
          !$omp parallel default(shared), private(tile, start_cell, stop_cell, cell, k, ik, df2, m2, df, m1)
          !$omp do schedule(static)
          !$acc parallel loop gang num_gangs(ntiles_per_colour) num_workers(tile_x*tile_y) &
          ...
          do tile = 1, ntiles_per_colour
             ...
             do cell = start_cell, stop_cell
                ...
                do k = 0, nlayers-1
                   ...
                   !$acc loop seq
                   do df2 = 1, ndf2
                      m2 = map2(df2, cmap(colour,cell))
                      !$acc loop seq
                      do df = 1,ndf1
                         m1 = map1(df, cmap(colour,cell))
                         !$acc atomic update
